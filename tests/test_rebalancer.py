...
class TestAllocateSell:
    ...
    def test_skips_account_with_only_sell_symbol(self):
        """Account whose only position is the sell symbol is excluded."""
        solo_acct = AccountInfo(
            number="33333", account_type="RRSP",
            client_account_type="Individual", owner="Carol",
            positions=[
                Position(
                    symbol="IVV", symbol_id=10, quantity=200,
                    market_value=150000.0, current_price=750.0, currency="USD",
                    account_number="33333", account_type="RRSP", owner="Carol",
                ),
            ],
        )
        trades = allocate_sell(
    ...
    def test_skips_account_where_all_others_also_overweight(self):
        """Account where every other position is also overweight is excluded."""
        acct = AccountInfo(
            number="44444", account_type="TFSA",
            client_account_type="Individual", owner="Dave",
            positions=[
                Position(
                    symbol="IVV", symbol_id=10, quantity=200,
                    market_value=150000.0, current_price=750.0, currency="USD",
                    account_number="44444", account_type="TFSA", owner="Dave",
                ),
                Position(
                    symbol="QQQ", symbol_id=11, quantity=100,
                    market_value=50000.0, current_price=500.0, currency="USD",
                    account_number="44444", account_type="TFSA", owner="Dave",
                ),
            ],
        )
        drifts = {"IVV": 2.0, "QQQ": 1.5}
        trades = allocate_sell(
            symbol="IVV",
            total_shares=50,
            price=750.0,
    ...
    def test_allows_sell_when_one_other_is_not_overweight(self):
        """Account with at least one non-overweight alternative is allowed."""
        acct = AccountInfo(
            number="44444", account_type="TFSA",
            client_account_type="Individual", owner="Dave",
            positions=[
